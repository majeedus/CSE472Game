...
: m_time(0)
{
	...
	COddSinesFactory* oddsinesfactory = new COddSinesFactory();
	CWaveInstrumentFactory* waveinstfactory = new CWaveInstrumentFactory();
	CPianoInstrumentFactory* pianoinstfactory = new CPianoInstrumentFactory();
	CToneInstrumentFactory* toneinstfactory = new CToneInstrumentFactory();
	CAdditiveFactory* 	additivefactory = new CAdditiveFactory();
	CSubtractiveFactory* subtractivefactory = new CSubtractiveFactory();

	CString msg = L"Loading Wave Files: ";
	AfxMessageBox(msg);

	pianoinstfactory->LoadFile(L"CompletePiano\\A4l.wav");
	pianoinstfactory->LoadFile(L"CompletePiano\\E4l.wav");
	pianoinstfactory->LoadFile(L"CompletePiano\\G4l.wav");
	pianoinstfactory->LoadFile(L"CompletePiano\\B3l.wav");
	pianoinstfactory->LoadFile(L"CompletePiano\\D3l.wav");
	pianoinstfactory->LoadFile(L"CompletePiano\\F3#l.wav");
	pianoinstfactory->LoadFile(L"CompletePiano\\E3l.wav");
	pianoinstfactory->LoadFile(L"CompletePiano\\C4l.wav");
	pianoinstfactory->LoadFile(L"CompletePiano\\A3l.wav");
	pianoinstfactory->LoadFile(L"CompletePiano\\G3l.wav");
	
	pianoinstfactory->LoadFile(L"CompletePiano\\pedald.wav");
	pianoinstfactory->LoadFile(L"CompletePiano\\pedalu.wav");

	m_InstrumentFactories.push_back(oddsinesfactory);
	m_InstrumentFactories.push_back(waveinstfactory);
	m_InstrumentFactories.push_back(pianoinstfactory);
	m_InstrumentFactories.push_back(toneinstfactory);
	m_InstrumentFactories.push_back(additivefactory);
	m_InstrumentFactories.push_back(subtractivefactory);
		// fill in with more factories if needed
		// all will be cleaned up in the destructor
...
CSynthesizer::~CSynthesizer()
{
	for (int i = 0; i < m_InstrumentFactories.size(); i++) {
		if (m_InstrumentFactories[i] == NULL)
			continue;

		delete m_InstrumentFactories[i];
}
}
...
void CSynthesizer::Start()
{
	...
	m_ReverbEffect.StreamBegin();
	m_FlangeEffect.StreamBegin();
	m_ChorusEffect.StreamBegin();
	m_CompressorEffect.StreamBegin();
}
...
bool CSynthesizer::Generate(double * frame)
{
	...
	while (m_currentNote < (int)m_notes.size())
	{
		...
		//CInstrument *instrument = NULL;
		for (auto factory : m_InstrumentFactories) {
			if (factory->InstrumentName() == note->Instrument()) {
	...
	for (list<CInstrument *>::iterator node = m_instruments.begin(); node != m_instruments.end();)
	{
		...
		{
			// If we returned true, we have a valid sample.  
			// Add it to the frame.
			for (int c = 0; c < GetNumChannels(); c++)
			{
				// process for effects
				m_EffectsModule.ProcessAudioNode(instrument, c);

				// add in the bypass audio...
				frame[c] += instrument->Frame(c) *instrument->GetBypassGain();
	...
	if (m_instruments.size() == 0) {
		for (int c = 0; c < GetNumChannels(); c++)
			m_EffectsModule.ProcessBlankFrame(c);
	}

	m_EffectsModule.AdvanceEffectStreams();

	// and the effect audio no matter what.
	for (int c = 0; c < GetNumChannels(); c++)
		frame[c] += m_EffectsModule.GetProcessedFrame(c);
...
void CSynthesizer::XmlLoadScore(IXMLDOMNode * xml)
{
	...
	for (; node != NULL; NextNode(node))
	{
		...
		if (name == L"instrumentparams")
		{
			XmlLoadInstrumentParams(node);
		}

		if (name == L"effectconfig") {
			XmlLoadEffectConfig(node);
		}

	}
}

void CSynthesizer::XmlLoadEffectConfig(IXMLDOMNode * xml) {
	...
	for (int i = 0; i<len; i++)
	{
		...
		if (name == "slot") {
			slot = (int)wcstol(value.bstrVal, NULL, 0);
		}
	}

	// put known effects into the configured slot
	if (effectName == L"Reverb") {
		m_EffectsModule.SetEffectSlot(&m_ReverbEffect, slot);
	}

	if (effectName == L"Chorus") {
		m_EffectsModule.SetEffectSlot(&m_ChorusEffect, slot);
	}

	if (effectName == L"Flange") {
		m_EffectsModule.SetEffectSlot(&m_FlangeEffect, slot);
	}

	if (effectName == L"Compressor") {
		m_EffectsModule.SetEffectSlot(&m_CompressorEffect, slot);
	}
}
...
void CSynthesizer::XmlLoadInstrumentParams(IXMLDOMNode * xml) {
	wstring instrument = L"";
	double bypassGain = 1.0;
	double effect0 = 0;
	double effect1 = 0;
	double effect2 = 0;
	double effect3 = 0;
	...
	for (int i = 0; i<len; i++)
	{
		...
		if (name == "bypassgain") {
			bypassGain = wcstod(value.bstrVal, NULL);
		}
		if (name == "effect0") {
			effect0 = wcstod(value.bstrVal, NULL);
		}
		if (name == "effect1") {
			effect1 = wcstod(value.bstrVal, NULL);
		}
		if (name == "effect2") {
			effect2 = wcstod(value.bstrVal, NULL);
		}
		if (name == "effect3") {
			effect3 = wcstod(value.bstrVal, NULL);
		}
	}

	// tell the appropriate factory about the gain parameters
	for (auto factory : m_InstrumentFactories) {
		if (factory->InstrumentName() == instrument) {
			factory->SetGainParams(bypassGain, effect0, effect1, effect2, effect3);
		}
	}
}
